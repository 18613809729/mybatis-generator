...
<mapper namespace="${mapperPackage}.${StringUtils.camelized(tableName)?cap_first}Mapper">
	<resultMap id="${StringUtils.camelized(tableName)}ResultMap" type="${domainPackage}.${StringUtils.camelized(tableName)?cap_first}">
  	<#list columns as column>
  		<result property="${column.propName}" column="${column.name}"<#if column.name == "CERT_NO"> typeHandler="com.nbs.jiaxiao.component.CryptTypeHandler"</#if>/>
  	</#list>
	</resultMap>
		...
		<#list columns as column>
			<#if column.name == "LAST_UPDATE_NO">
	 		...
	 		<#elseif column.name == "CREATED_TIME" || column.name == "MODIFIED_TIME" >
	 			...
	 			<if test="param.${column.propName}End != null"> AND ${column.name} <![CDATA[<=]]> ${"#{"}param.${column.propName}End} </if>
	 		<#else>
	 			<if test="${column.propName} != null"> AND ${column.name} = ${"#{"}${column.propName}<#if column.name == "CERT_NO">, typeHandler=com.nbs.jiaxiao.component.CryptTypeHandler</#if>}</if> 
	 		</#if>
    	</#list>
	</sql>
	...
	<insert id="insert" useGeneratedKeys="true" keyProperty="id">
		...
		<trim prefix="(" suffixOverrides="," suffix=")">
		<#list columns as column>
			<if test="${column.propName} != null">${"#{"}${column.propName}<#if column.name == "CERT_NO">, typeHandler=com.nbs.jiaxiao.component.CryptTypeHandler</#if>},</if> 
		</#list>
		</trim>
	</insert>
	...
	<update id="updateByPriKey">
		...
		<#list columns as column>
			...
			<#elseif column.name != "ID">
				<if test="${column.propName} != null">${column.name}=${"#{"}${column.propName}<#if column.name == "CERT_NO">, typeHandler=com.nbs.jiaxiao.component.CryptTypeHandler</#if>},</if>
			</#if>
		 </#list>
    	</set>
